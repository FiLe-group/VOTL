...
public class TicketCountCmd extends CommandBase {
	...
	protected void execute(SlashCommandEvent event) {
		...
		Integer countRoles = bot.getDBUtil().tickets.countTicketsByMod(event.getGuild().getIdLong(), user.getIdLong(), afterTime, beforeTime, true);
		Integer countOther = bot.getDBUtil().tickets.countTicketsByMod(event.getGuild().getIdLong(), user.getIdLong(), afterTime, beforeTime, false);
