...
public class SetNameCmd extends SlashCommand {
	...
	public SetNameCmd(App bot) {
		this.name = "setname";
		this.help = bot.getMsg("0", "bot.voice.setname.description");
		this.category = new Category("voice");
		this.userPerms = new Permission[]{Permission.MANAGE_SERVER};
		this.options = Collections.singletonList(
			new OptionData(OptionType.STRING, "name", bot.getMsg("0", "bot.voice.setname.option_description"))
				.setRequired(true)
		);
		this.bot = bot;
	}
	...
	private MessageEditData getReply(SlashCommandEvent event, String filName) {
		...
		bot.getDBUtil().guildVoiceSetName(event.getGuild().getId(), filName);

		return MessageEditData.fromEmbeds(
			bot.getEmbedUtil().getEmbed(event.getMember())
				.setDescription(bot.getMsg(event.getGuild().getId(), "bot.voice.setname.done").replace("{value}", filName))
				.build()
		);
	}
}
